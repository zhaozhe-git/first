<template>
  <div class="calendar">
    <div class="head">
      <el-button @click="onPreMonth" type="text" icon="el-icon-caret-left"></el-button>
      <span class="date">{{curYear + "年" + curMonth + "月"}}</span>
      <el-button @click="onNextMonth" type="text" icon="el-icon-caret-right"></el-button>
    </div>
    <div class="body">
      <div class="title">
...
export default {
  name: 'calendar',
  props: {
    courseDate: { //含有课程的日期集合 如['2019-8-1', '2019-8-15']
      type: Array,
      default() {
        return []
      }
  ...
  computed: {
    courseDateSet() {
      ...
      for (let i = 0; i < this.courseDate.length; ++i) {
        retAry.push(new Date(this.courseDate[i]))
      }
      return retAry
    }
  },
  ...
  created() {
    ...
    this.$emit(this.event.onCurDate, `${this.curYear}-${this.curMonth}-${this.nowDate.getDate()}`)
  ...
  methods: {
    ...
    onPreMonth() {
      ...
      this.selectDay = 0
      this.init(new Date(`${this.curYear}-${this.curMonth}`))
    ...
    onClickDay(item) {
      ...
      this.$emit(this.event.onClickDay, `${this.curYear}-${this.curMonth}-${item.day}`)
    },
    // other
    init(date) {
      // current
